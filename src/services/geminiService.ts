...
export async function fetchPubgNews(): Promise<NewsItem[]> {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `أعطني آخر 8 أخبار وفعاليات وتحديثات وصيانة مجدولة وعروض شدات (UC) وأطوار جديدة وتسريبات للموسم القادم للعبة ببجي موبايل بتاريخ اليوم ${today}.
...
export async function analyzeClipWithAI(title: string, description: string): Promise<{
  ...
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `أنت خبير ومحلل محترف في لعبة ببجي موبايل (PUBG Mobile).
